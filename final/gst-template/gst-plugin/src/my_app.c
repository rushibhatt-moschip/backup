...
main (int   argc, char *argv[])
{
	...
	/* Check input arguments */
	 if (argc != 2) {
	 g_printerr ("Usage: ./a.out <row number>\n");
	 return -1;
	 }
	 gint a=atoi(argv[1]);
	g_print("%d",a);
...
//		g_object_set(autovideosink, "text-overlay", 0, NULL);
	...
	//	g_print ("Now playing: \n"); //argv[1]);
		...
		g_print("width= %d, height= %d,format= %s,rate= %d/1,"
				"size= %d, x_cor= %d, y_cor= %d, name= %s,"
				"color= %s, fil_mode= %s, x_copy= %d,"
			       "y_copy= %d, alpha= %d, strict= %d, \nresult= %s\n", width, height, format, rate, size, 
				x_cor, y_cor, name, color,fil_mode, x_copy, y_copy, alpha, strict, array[var]);
